...
int main (int argc, char* argv[]) {
    ...
    /*
    cout << "Salida: <k> <n> <t_algPropuesto> <t_algNuestro> <t_algPropuesto> <t_algPropuesto>"
    */
    int k = atoi(argv[1]);
    int n = atoi(argv[2]);
    //int n = 10, k = 10;
...
void algoritmoNuestro (int **vectores, int k, int n) {
    ...
    int tamanios [k];
    for (int i = 0; i < k; i++) tamanios[i] = n;	// Rellenamos un vector de tamaños
    int i, n_vec = k, *auxv;				// inicialmente con todos a n (tamaño dado)
    
    while (n_vec > 1){					// No paramos hasta que al reducir no nos 
    							// quede un único vector
        for (i = 0; i < n_vec-1; i += 2){
            
            // Llamamos a la función pasándo como parámetros 2 vectores concatenados y sus
            // respectivos tamaños guardados en el vector donde se guardan todos los tamaños.
            // Guardamos el vector resultante en la posición i/2 (al ir avanzando de 2 en 2)
            // del array de vectores.
            // Posteriormente actualizamos el tamaño del vector resultante
            
            MezclaVec(vectores[i], vectores[i+1], tamanios[i], tamanios[i+1], vectores[i/2]);
            tamanios[i/2] = tamanios[i] + tamanios[i+1];
        }
        
        if (i == n_vec-1){				// Si ha habido un vector que no ha sido 
        						// juntado ya que el número de vectores 
            auxv = vectores[i/2];			// era impar añadimos dicho vector sobrante
            vectores[i/2] = vectores[n_vec-1];	// a la cola de todos los vectores con su
            vectores[n_vec-1] = auxv;			// correspondiente tamaño, que será la mitad
            tamanios[i/2] = tamanios[n_vec-1];	// que el resto.


            n_vec /= 2;				// Dividimos entre 2 el número de vectores
            n_vec ++;					// Sumamos uno al haber sido impar
            
        } 
        else n_vec /= 2;				// Dividimos entre 2 el número de vectores
    }
}
