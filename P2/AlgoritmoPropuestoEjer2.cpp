...
/*for (int i=0; i<k; i++) {
 ...
 cout << " " << endl;
    ...
    auxv = vectordevectores.back();
    vectordevectores.pop_back();
    while (vectordevectores.size() != 0){
        auxv = MezclaVec(auxv, vectordevectores.back());
        vectordevectores.pop_back();
    }
    tdespues = chrono::high_resolution_clock::now();
    transcurrido = chrono::duration_cast<chrono::duration<double>>(tdespues-tantes);

    /*for (vector<int>::iterator i = auxv.begin(); i != auxv.end(); ++i)
        cout << " " << *i;*/

    cout << "Tam: " << auxv.size() << "\t" << transcurrido.count() << "\n";
}