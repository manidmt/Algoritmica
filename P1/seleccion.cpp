...
int main(int argc, char * argv[])
{
  ...
  int iteraciones = 15;
  double media = 0, min = 100000, max = 0;
  ...
  for (int j=0; j<iteraciones; j++) {
    ...
    if (transcurrido.count() < min)
      min = transcurrido.count();
    
    if (transcurrido.count() > max)
      max = transcurrido.count();

    media += transcurrido.count();
  }
  media /= iteraciones;

  cout << n << "\t" << media << "\t" << min << "\t" << max << endl;
